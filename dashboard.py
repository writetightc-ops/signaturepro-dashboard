...
_FALLBACK_CAL_RATES = {
    ...
    "OPTNEW":       {"варианты": 300, "обучение": 400, "бонус_без_правок": 200, "тарифный_бонус":   0, "правка": 75},
    "PRNEW":        {"варианты": 500, "обучение": 200, "бонус_без_правок": 250, "тарифный_бонус": 300, "правка": 75},
    "ДОП_ОБУЧЕНИЕ": {"варианты":   0, "обучение": 200, "бонус_без_правок":   0, "тарифный_бонус":   0, "правка":  0},
    "ДОП_ПОДПИСЬ":  {"варианты": 250, "обучение": 200, "бонус_без_правок": 100, "тарифный_бонус":   0, "правка": 75},
}
_FALLBACK_MGR_RATES = {
...
_PUNCT = ".,;:!?- \"'«»()[]"
_TRANS_PUNCT = {ord(c): None for c in _PUNCT}


def _norm_name(text: str) -> str:
    """Нормализация имени для дедупликации: lower + схлопывание пробелов +
    удаление краевой пунктуации. «Иван  Иванов.» → «иван иванов»."""
...
def calc_order_earnings(order, date_from, date_to, cal_rates, mgr_rates):
    ...
    - Варианты + тарифный бонус → когда дата «Варианты» попадает в период.
      Тарифный бонус достаётся каллиграфу, который готовил варианты
      (тому, кто указан в строке с проставленной датой «Варианты»).
    - Правки       → каждая правка, чья дата попадает в период.
    - Обучение     → когда дата «Обучение» попадает в период.
    - Бонус без правок → при завершении заказа (галка в столбце B И дата
                          «Завершение» в периоде) И если по заказу ВООБЩЕ нет
                          правок (независимо от периода).
    - Менеджерский бонус → при завершении заказа (B=TRUE И дата «Завершение»
                          в периоде). Дедупликация «один раз на клиента»
                          выполняется в calculate_salary. Для тарифа
                          ДОП_ОБУЧЕНИЕ ставка менеджера = 0.
    """
    tariff = order["тариф"]
    ...
    # Завершение заказа = галка в столбце B (статус) И дата завершения в периоде
    completed = bool(order["статус"]) and in_p(order["завершение"])
    ...
    if completed:
        ...
        if total_правок == 0 and rates["бонус_без_правок"] > 0:
...
def calculate_salary(date_from, date_to):
    ...
    results = {}
    # Менеджер получает бонус за клиента один раз за период.
    # Дедупликация по двум ключам: нормализованное имя клиента И ссылка на
    # папку (столбец Q) — совпадение любого из двух считается повтором.
    mgr_paid_name = set()  # {(norm менеджер, norm клиент)}
    mgr_paid_link = set()  # {(norm менеджер, ссылка)}
    ...
    for order in all_orders:
        ...
        if cal_name and cal_total > 0:
            ...
            if order["статус"] and order["завершение"] and date_from <= order["завершение"] <= date_to:
        ...
        if mgr_name and mgr_bonus > 0:
            m_key = _norm_name(mgr_name)
            c_key = _norm_name(order["клиент"])
            link  = order.get("ссылка", "").strip()
            is_dup = (m_key, c_key) in mgr_paid_name or (
                bool(link) and (m_key, link) in mgr_paid_link
            )
            if is_dup:
                mgr_bonus = 0.0
            else:
                mgr_paid_name.add((m_key, c_key))
                if link:
                    mgr_paid_link.add((m_key, link))
