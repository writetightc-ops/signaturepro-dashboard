...
def _row_to_order(row, sheet_name):
    ...
    return {
        ...
        # Новые столбцы T–W (ОС 5, Правка 5, ОС 6, Правка 6)
        "ос5":         _parse_date(r[19]),
        "правка5":     _parse_date(r[20]),
        "ос6":         _parse_date(r[21]),
        "правка6":     _parse_date(r[22]),
        "_sheet":      sheet_name,
    }
...
def transfer_orders(spreadsheet_url, credentials_file, from_sheet_name, to_sheet_name):
    ...
    # Добавляем в целевой лист одним батчевым запросом (A–X = 24 столбца)
    padded_rows = [(row + [""] * 24)[:24] for row in rows_to_move]
